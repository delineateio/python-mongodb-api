...
class Repository:
    ...
    @retry(wait=wait_exponential(multiplier=0.1), stop=stop_after_attempt(3), reraise=True)
    ...
    def startup(self) -> None:
        ...
        try:
            ...
            if "handle" not in collection.list_indexes():
                collection.create_index([ ("handle", ASCENDING) ], name="handle", unique=True)
    ...
    @retry(retry=retry_if_not_exception_type(RepositoryNotFoundError),
    ...
    @retry(retry=retry_if_not_exception_type(RepositoryDuplicateKeyError),
        wait=wait_exponential(multiplier=0.1),
        stop=stop_after_attempt(3),
        reraise=True)
