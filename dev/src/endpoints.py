...
@router.delete("/reset", status_code=status.HTTP_205_RESET_CONTENT, response_class=Response)
@inject
...
@inject
async def get_customers(repository: Repository = Depends(Provide[Container.repository])):
...
@inject
async def get_customer(handle : str,
                        repository: Repository = Depends(Provide[Container.repository])):
...
@inject
async def update_customer(handle : str,
                            repository: Repository = Depends(Provide[Container.repository])):
...
@inject
def get_relationships(handle : str,
                        repository: Repository = Depends(Provide[Container.repository])):
...
@router.post("/customers/{code}/relationship",
            status_code=status.HTTP_201_CREATED, response_class=Response)
@inject
async def create_relationship(handle: str, relationship: Relationship,
                                repository: Repository = Depends(Provide[Container.repository])):
